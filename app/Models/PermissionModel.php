...
class PermissionModel extends Model
{
    ...
    public function findPermissionByCodePermission($codePermission)
    {
        $permission = $this
            ->asArray()
            ->where(['permission' => $codePermission])
            ->first();

        if (!$permission) throw new Exception('Could not find Permission for specified code');
